...
- OpenClaw
- 源码解析
cover: https://file-1305436646.cos.ap-nanjing.myqcloud.com/blog/banner/2026-03-14.webp
---

# OpenClaw 设计解析（七）：插件系统

> 本篇聚焦 OpenClaw 的插件系统：插件包长什么样、如何被发现和加载、为什么 Hook 与 slot 是两个关键机制，以及这些设计背后的工程取舍。

## 1. 先看整体：OpenClaw 的插件不是“动态 import 一下”
...
> **插件的职责是把能力注册进系统；系统真正用的是统一的 `PluginRegistry`。**
...
这里可以看到：**插件的“种类”不是死板 class hierarchy，而是注册出来的能力组合。**
...
### 2.3 通道插件：把外部消息面接进 OpenClaw

这类插件比较直接，本质上是在做“渠道接入”。  
它们最核心的动作通常就是：
...
这两个插件不只是“多一个工具”这么简单，它们还在定义：

> **OpenClaw 的长期记忆主要由谁实现。**

这也是为什么后面会有 `memory slot` 机制，强制只选一个 owner。

### 2.5 模型 / Provider / Auth 插件：给模型接入补登录和配置
...
### 2.6 工具 / 工作流 / Hook 插件：直接增强 agent 能力
...
所以在 OpenClaw 里，“插件系统”就是一套统一的扩展机制，不只是“agent tool marketplace”。

## 3. `PluginRegistry`：统一的能力表
