...
### 4.1 openclaw doctor：30 步自动修复引擎

`doctor` 涉及 15+ 个文件，实现跨度很大。它按固定顺序执行约 30 项检查，每项检查都可能触发自动修复。通过 `--fix` 自动确认所有修复，`--force` 启用激进修复（如覆盖自定义服务配置），`--non-interactive` 抑制所有交互提示。
...
### 4.2 openclaw gateway run：信号循环与优雅重启
